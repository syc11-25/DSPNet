...
if __name__=="__main__":
    ...
    ## Load trained model
    initial_epoch = findLastCheckpoint(save_dir="./Checkpoint/f4/Model_DSP")
    if initial_epoch > 0:
        print('resuming by loading epoch %04d' % initial_epoch)
        model = torch.load(os.path.join("./Checkpoint/f4/Model_DSP", 'model_%04d.pth' % initial_epoch))
    ...
    for epoch in range(initial_epoch, 100):
        ...
        torch.save(model, os.path.join("./Checkpoint/f4/Model_DSP", 'model_%04d.pth' % (epoch + 1)))  # save model
